...
import { addToCartAction, getCartAction, removeFromCartAction } from '@/app/actions';
          ...
          <div className="relative w-full max-w-md bg-white h-full shadow-2xl p-6 flex flex-col animate-in slide-in-from-right duration-300">
             <div className="flex justify-between items-center mb-8 border-b border-brand-100 pb-4">
               <h2 className="font-display text-2xl italic text-brand-900 flex items-center gap-2">
                 Your Cart
                 {loading && <Loader2 className="animate-spin h-4 w-4 text-brand-400" />}
               </h2>
               <button onClick={closeCart} className="text-brand-400 hover:text-brand-900">Close</button>
             </div>
             ...
             {!cart || cart.totalQuantity === 0 ? (
               <div className="flex-1 flex flex-col items-center justify-center text-brand-400 font-light gap-4">
                 ...
                 <button onClick={closeCart} className="text-brand-900 underline font-medium text-sm">
                   Continue Shopping
                 </button>
               </div>
             ) : (
                 ...
                 {cart.lines.edges.map(({ node: item }) => (
                   <div key={item.id} className="flex gap-4">
                     ...
                     <div className="flex-1 flex flex-col justify-between py-1">
                       <div>
                         <h3 className="font-display italic text-brand-900">{item.merchandise.product.title}</h3>
                         <p className="text-xs text-brand-500">{item.merchandise.title !== 'Default Title' ? item.merchandise.title : ''}</p>
                       </div>
                       <div className="flex justify-between items-center">
                         <span className="font-mono text-sm">{item.cost.totalAmount.currencyCode} {parseFloat(item.cost.totalAmount.amount).toFixed(2)}</span>
                         <button 
             ...
             {cart && cart.totalQuantity > 0 && (
               <div className="mt-auto border-t border-brand-100 pt-6 space-y-4">
                 <div className="flex justify-between text-brand-900 font-bold">
                   <span>Subtotal</span>
                   <span>{cart.cost.subtotalAmount.currencyCode} {parseFloat(cart.cost.subtotalAmount.amount).toFixed(2)}</span>
                 </div>
                 <p className="text-xs text-brand-400 text-center">Shipping & taxes calculated at checkout.</p>
